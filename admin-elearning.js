...
const courses = [
  {id:1, tag:'Soil Science',     tagColor:'#2D5A27', accent:'#2D5A27', title:'Soil Health Assessment Fundamentals',  desc:'Learn how to assess soil quality, understand pH levels, and optimize soil conditions for different crops.',                                       hours:'2 hours',   modules:'6 modules',  progress:100, status:'completed'  },
  {id:2, tag:'Farming Methods',  tagColor:'#f59e0b', accent:'#f59e0b', title:'Organic Farming Techniques',            desc:'Master organic farming methods including composting, natural pest control, and crop rotation strategies.',                                         hours:'3 hours',   modules:'8 modules',  progress:63,  status:'in-progress'},
  {id:3, tag:'Water Management', tagColor:'#3b82f6', accent:'#3b82f6', title:'Irrigation Systems & Water Management', desc:'Understand different irrigation methods, water conservation techniques, and smart irrigation scheduling.',                                     hours:'2.5 hours', modules:'7 modules',  progress:0,   status:'not-started'},
  {id:4, tag:'Crop Protection',  tagColor:'#ef4444', accent:'#ef4444', title:'Pest & Disease Identification',          desc:'Identify common crop pests and diseases. Learn prevention and treatment strategies.',                                                           hours:'2 hours',   modules:'5 modules',  progress:0,   status:'not-started'},
  {id:5, tag:'Sustainability',   tagColor:'#22c55e', accent:'#22c55e', title:'Sustainable Agriculture Practices',     desc:'Explore sustainable farming practices aligned with SDG 2 and SDG 12 for responsible production.',                                               hours:'4 hours',   modules:'10 modules', progress:100, status:'completed'  },
  {id:6, tag:'Safety',           tagColor:'#f59e0b', accent:'#f59e0b', title:'Farm Equipment Safety & Maintenance',   desc:'Safety protocols for operating farm equipment and routine maintenance procedures.',                                                         hours:'1.5 hours', modules:'4 modules',  progress:50,  status:'in-progress'},
];
...
function courseBtn(c){
  ...
  if(c.status==='in-progress') return`<button class="btn btn-primary" style="width:100%;justify-content:center;background:${c.accent};">Continue &nbsp;▷</button>`;
...
function renderElearning(){
  const vis = courses.filter(c =>
    elFilter==='All'          ? true :
    elFilter==='In Progress'  ? c.status==='in-progress' :
    elFilter==='Completed'    ? c.status==='completed' :
  ...
  document.getElementById('pageContent').innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1rem;margin-bottom:1.25rem;">
      <div class="card" style="display:flex;align-items:center;gap:0.75rem;">
        ...
        <div><div style="font-size:1.5rem;font-weight:700;line-height:1;">${completed}</div><div style="font-size:0.72rem;color:var(--muted);">Courses Completed</div></div>
      </div>
      <div class="card" style="display:flex;align-items:center;gap:0.75rem;">
        ...
        <div class="card" style="padding:0;overflow:hidden;display:flex;flex-direction:column;">
          ...
          <div style="padding:1.25rem;flex:1;display:flex;flex-direction:column;">
            ...
            <div class="progress-wrap" style="margin-bottom:0.85rem;">
              ...
              <div class="progress-bar"><div class="progress-fill" style="width:${c.progress}%;background:${c.accent};"></div></div>
            </div>
            ${courseBtn(c)}
          </div>
        </div>`).join('')}
