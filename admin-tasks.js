document.addEventListener('DOMContentLoaded', async function () {
  ...
  let tasks      = [];
  let WORKERS    = []; // { id, name } from workers table
  let nextId     = null;
  let editingId  = null;
  let searchQ    = '';
  let filterVal  = 'all';
  ...
  async function loadWorkers() {
    const { data, error } = await window.db
      ...
      .eq('status', 'active')
      .order('name');
    if (error) { console.error('[Tasks] load workers:', error.message); return; }
    WORKERS = data || [];
  }
  ...
  async function loadTasks() {
    ...
    if (error) { console.error('[Tasks] load tasks:', error.message); return; }
    tasks = (data || []).map(t => ({
      ...t,
      worker:      t.workers?.name || t.worker || '—',
      desc:        t.description   || '',
      due:         t.due_date      || '',
  ...
  window.AT = {
    ...
    openEdit(id) {
      ...
      set('mTitle', t.title);
      set('mDesc', t.desc);
      set('mPriority', t.priority);
      // Set worker by worker_id (the select uses worker id as value)
      set('mWorker', t.worker_id || '');
      set('mDue', t.due);
    ...
    async submit() {
      ...
      const workerId = document.getElementById('mWorker')?.value      || '';
      ...
      show('mWorkerErr', !workerId);
      show('mDueErr',    !due);
      if (!title || !workerId || !due) return;

      // Get worker name for display
      const workerObj = WORKERS.find(w => String(w.id) === String(workerId));
    ...
    async complete(id) {
      const { error } = await window.db
        ...
        .eq('id', id);
      if (error) { alert('Failed to complete task: ' + error.message); return; }
      await loadTasks();
      render();
    },
    ...
    async reopen(id) {
      const { error } = await window.db
        .from('tasks')
        .update({ completed: false, status: 'pending' })
        .eq('id', id);
      if (error) { alert('Failed to reopen task: ' + error.message); return; }
      await loadTasks();
      render();
    },
  ...
  // ── Real-time: admin sees worker completions instantly ────────
  window.db.channel('admin-tasks-live')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'tasks' },
      async () => { await loadTasks(); render(); })
    .subscribe();
