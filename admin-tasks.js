document.addEventListener('DOMContentLoaded', function () {
  ...
  let tasks = [
    { id:1, priority:'High',   completed:false, title:'Irrigate Rice Paddies – Section A', desc:'Complete irrigation cycle for rice paddies in Section A. Ensure water level reaches 5cm depth.',   due:'2026-03-06', worker:'Juan Dela Cruz' },
    { id:2, priority:'Medium', completed:true,  title:'Apply Fertilizer to Corn Fields',   desc:'Apply NPK fertilizer (14-14-14) at recommended rate of 2 bags per hectare.',                       due:'2026-03-05', worker:'Maria Clara'   },
    { id:3, priority:'Low',    completed:false, title:'Check Pest Traps',                  desc:'Inspect and replace pest traps in the vegetable garden area. Record pest count.',                   due:'2026-03-07', worker:'Pedro Santos'  },
    { id:4, priority:'High',   completed:false, title:'Harvest Ripe Tomatoes',             desc:'Harvest tomatoes that have reached maturity in greenhouse section B.',                               due:'2026-03-08', worker:'Carlos Garcia' },
  ];
  let nextId    = 5;
  let editingId = null;
  let searchQ   = '';
  ...
  const WORKERS = ['Juan Dela Cruz','Maria Clara','Pedro Santos','Carlos Garcia','Ana Reyes'];
    ...
    const taskCards = filtered.length ? filtered.map(t => `
      <div class="card" style="opacity:${t.completed ? '.75' : '1'};">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:.5rem;margin-bottom:.45rem;">
          ...
          <span class="badge ${PM[t.priority].badge}" style="flex-shrink:0;">${t.priority}</span>
        </div>
  ...
  window.AT = {
    ...
    submit() {
      ...
      const desc   = document.getElementById('mDesc')     ? document.getElementById('mDesc').value.trim() : '';
      const prio   = document.getElementById('mPriority') ? document.getElementById('mPriority').value    : 'Medium';
      const worker = document.getElementById('mWorker')   ? document.getElementById('mWorker').value      : '';
      const due    = document.getElementById('mDue')      ? document.getElementById('mDue').value         : '';

      const show = (id, show) => { const el = document.getElementById(id); if (el) el.style.display = show ? 'block' : 'none'; };
      ...
      if (editingId) {
        ...
        tasks.push({ id: nextId++, title, desc, priority: prio, worker, due, completed: false });
      }
    ...
    complete(id) { const t = tasks.find(t => t.id === id); if (t) { t.completed = true;  render(); } },
    reopen(id)   { const t = tasks.find(t => t.id === id); if (t) { t.completed = false; render(); } },
    del(id)      { if (!confirm('Delete this task?')) return; tasks = tasks.filter(t => t.id !== id); render(); },
  };
