...
function renderPage() {
  ...
  const todayLog = AttendanceStore.getTodayLog();
  const logs     = AttendanceStore.getLogs();
...
function handleTimeIn() {
  if (AttendanceStore.isTimedInToday() || AttendanceStore.isTimedOutToday()) return;
  if (!geofenceInside) {
    if (!confirm('You are outside the farm area. Your location will be flagged. Time in anyway?')) return;
  }
  const result = AttendanceStore.timeIn();
...
function handleTimeOut() {
  ...
  const result = AttendanceStore.timeOut();
  if (!result.success) { alert(result.error); return; }
  renderPage();
}
