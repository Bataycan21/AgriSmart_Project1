document.addEventListener('DOMContentLoaded', async function () {
  ...
  const CROP_ICONS = { Rice:'🌾', Corn:'🌽', Vegetables:'🥬', Sugarcane:'🎋', Banana:'🍌', Coconut:'🥥' };
  const CROP_BASE  = {
  ...
  async function loadHistory() {
    const { data, error } = await window.db
      ...
      .limit(20);
    if (error) { console.error('[AI] load history:', error.message); return; }
    historyList = (data || []).map(p => ({
      ...
      province:   p.province,
      soil:       p.soil_data,
      perHa:      p.per_ha_yield,
      total:      p.total_yield,
      unit:       CROP_BASE[p.crop]?.unit || 'tons/ha',
      confidence: p.confidence,
      water:      p.water_req,
      fertilizer: p.fertilizer,
      soilMatch:  p.soil_data || 'Good',
      tips:       p.tips       || [],
      schedule:   p.schedule   || [],
      risks:      p.risks      || [],
      raw:        p.gemini_raw,
      createdAt:  p.created_at ? new Date(p.created_at).toLocaleDateString('en-US', { year:'numeric', month:'short', day:'numeric' }) : '—',
...
Return ONLY this exact JSON structure:
{
  ...
  function fallbackCalc(crop, land, season, soil) {
    ...
    return {
      perHa: base.base * smod * soilM, confidence: 78,
      water: base.water, fertilizer: base.fertilizer,
          ...
          <div style="display:flex;flex-direction:column;gap:.9rem;">
            ...
            <div>
              ...
              <div style="position:relative;">
                ...
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--muted)" stroke-width="2" style="position:absolute;right:.75rem;top:50%;transform:translateY(-50%);pointer-events:none;"><polyline points="6 9 12 15 18 9"/></svg>
              </div>
            </div>
            <div>
              <label style="font-size:.8rem;font-weight:600;display:block;margin-bottom:.4rem;">Province</label>
              <div style="position:relative;">
            ...
            <div>
              <label style="font-size:.8rem;font-weight:600;display:block;margin-bottom:.4rem;">Soil Type</label>
              <input id="ai_soil" type="text" value="${aiForm.soil}"
                oninput="aiForm.soil=this.value" placeholder="e.g. Clay loam, Sandy loam"
                style="width:100%;padding:.6rem .75rem;border:1.5px solid var(--border);border-radius:8px;font-family:'Poppins',sans-serif;font-size:.82rem;outline:none;box-sizing:border-box;background:#f9fafb;"/>
            </div>
            <button class="btn btn-primary" onclick="AI.generate()"
    ...
    return `
      <div class="card" style="min-height:400px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1rem;padding:2rem;">
        ...
        <div style="text-align:center;">
          ...
          <div style="font-size:.78rem;color:var(--muted);line-height:1.6;">Analyzing crop conditions, seasonal patterns,<br>and regional data for your farm</div>
        </div>
      ...
      <div class="card" style="animation:fadeIn .4s ease;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;">
          ...
          <div style="display:flex;align-items:center;gap:.5rem;">
            <div style="background:${confBg};color:${confColor};padding:.3rem .75rem;border-radius:999px;font-size:.72rem;font-weight:700;">${r.confidence}% Confidence</div>
    ...
    return `
      <div class="card" style="padding:0;overflow:hidden;">
        <table>
          <thead>
            <tr>
              <th style="padding-left:1.5rem;">Crop</th><th>Area</th><th>Season</th>
              <th>Yield</th><th>Confidence</th><th>Date</th>
            ...
            ${historyList.map(p => {
              ...
              return `
                <tr>
                  ...
                  <td style="font-weight:700;font-size:.88rem;color:var(--green-dark);">${parseFloat(p.total).toFixed(1)} t</td>
                  <td>
      ...
      <div id="detailBackdrop" style="position:fixed;inset:0;background:rgba(0,0,0,.4);z-index:300;display:flex;align-items:center;justify-content:center;padding:1rem;">
        <div style="background:white;border-radius:16px;padding:1.75rem 2rem;width:680px;max-height:88vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.2);animation:modalIn .22s ease;" onclick="event.stopPropagation()">
          ...
          <div style="background:linear-gradient(135deg,#2D5A27,#3a7a32);border-radius:12px;padding:1.25rem;text-align:center;margin-bottom:1.25rem;color:white;">
            ...
            <div style="font-size:2.4rem;font-weight:700;line-height:1;">${parseFloat(p.total).toFixed(1)} <span style="font-size:1rem;font-weight:400;">tons</span></div>
            <div style="font-size:.78rem;opacity:.75;margin-top:.25rem;">${parseFloat(p.perHa).toFixed(2)} ${p.unit} · Confidence: <strong>${p.confidence}%</strong></div>
          </div>
  ...
  window.AI = {
    ...
    async generate() {
      ...
      try {
        ...
        let gd;
        try   { gd = await callGemini(aiForm.crop, area, aiForm.season, aiForm.soil, aiForm.province); }
        catch(e) { console.warn('Gemini failed, using fallback:', e.message); gd = fallbackCalc(aiForm.crop, area, aiForm.season, aiForm.soil); }
    ...
    async savePrediction() {
      if (!aiResult) return;
      const { error } = await window.db.from('ai_predictions').insert({
        crop:         aiResult.crop,
        land_area:    aiResult.area,
        season:       aiResult.season,
        soil_data:    aiResult.soil,
        province:     aiResult.province,
        total_yield:  aiResult.total,
        per_ha_yield: aiResult.perHa,
        confidence:   aiResult.confidence,
        fertilizer:   aiResult.fertilizer,
        water_req:    aiResult.water,
        tips:         aiResult.tips,
        schedule:     aiResult.schedule,
        risks:        aiResult.risks,
        gemini_raw:   aiResult.raw,
      ...
      if (btn) {
        ...
        setTimeout(async () => { await loadHistory(); renderAI(); }, 1200);
      } else {
        await loadHistory(); renderAI();
      }
    },

    async deletePred(id) {
      if (!confirm('Delete this prediction from history?')) return;
      const { error } = await window.db.from('ai_predictions').delete().eq('id', id);
